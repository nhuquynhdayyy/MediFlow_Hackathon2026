...
class TriageAgentService:
    def _build_messages(self, message: str, history: list) -> list:
        """Build message list với system prompt và history đã được chuẩn hóa."""
        weekdays = ["Thứ Hai", "Thứ Ba", "Thứ Tư", "Thứ Năm", "Thứ Sáu", "Thứ Bảy", "Chủ Nhật"]
        now = datetime.now()
        date_info = (
            f"\n\nNGÀY HIỆN TẠI: {weekdays[now.weekday()]}, {now.strftime('%Y-%m-%d')} "
            f"(giờ: {now.strftime('%H:%M')}). Dùng thông tin này khi bệnh nhân nói "
            "'ngày mai', 'tuần sau', v.v."
        )
    ...
    async def chat(
        self, message: str, history: list, api_key: str, model: str = "Llama-3.3-70B-Instruct"
    ) -> dict:
